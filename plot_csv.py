...
from matplotlib.ticker import FixedFormatter, ScalarFormatter, NullFormatter, FixedLocator, NullLocator, LogFormatterSciNotation
...
single = single_raw.pivot(index='dofs', columns='solver name', values='runtime')
# Normal
single.plot(legend=False, ax=ax1[0])
ax1[0].set_ylim([0, 220])
ax1[0].set_ylabel('runtime (s)')
ax1[0].xaxis.set_major_formatter(LogFormatterSciNotation())
# Log
single.plot(logx=True, logy=True, ax=ax1[1])
ax1[1].yaxis.set_major_formatter(ScalarFormatter())
# ~ ax1[1].yaxis.set_minor_formatter(NullFormatter())
# Comparison lines
...
ax1[1].legend(title='solver name', labels=single.columns)
fig1.suptitle('Computational cost of solvers, single core on ARCHER2')
...
weak = weak_raw.pivot(index='cores', columns='solver name', values='runtime')
weak.plot(logx=True, ax=ax2)
...
strong = strong_raw.pivot(index='cores', columns='solver name', values='runtime')
strong.plot(logx=True, logy=True, ax=ax3)
